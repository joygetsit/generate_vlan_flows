...
# Command to run: "./vlan_tagged_packets_create.py enp1s0f0 hi 10"
# Current Command to run: "./vlan_tagged_packets_create.py enp1s0f0"
...
def main():
    ...
    writetoPcap = PcapWriter('Mixed_VLAN_pkts.pcap')  # opened file to write
    ...
    for PktIP_lastoctet in range(2):
        #for pktnum in range(int(sys.argv[3])):
        for PktSeqNo in range(65535):
            ...
            #pkt_vlan2 = Ether(src=get_if_hwaddr(iface), dst=dstmac) / Dot1Q(vlan=2) / IP(src=ipsrcaspktsequencenumber, id=pktsequencenumber, proto=253)
